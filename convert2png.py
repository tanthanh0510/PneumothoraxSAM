...
def argparser():
    ...
    parser.add_argument('--train-path', default=os.path.join('siim',
                        'dicom-images-train'), type=str, help='Directory to training path')
    parser.add_argument('--test-path', default=os.path.join('siim',
                        'dicom-images-test'), type=str, help='Directory to testing path')
    parser.add_argument('--rle-path', default=os.path.join('siim',
                        'train-rle.csv'), type=str, help='Path to rle csv file')
    parser.add_argument('--output-path', default='dataset',
                        type=str, help='Path for saving dataset')
    parser.add_argument('--image-size', default=1024,
                        type=int, help='Converted image size')
    parser.add_argument('--number-threads', default=-1,
                        type=int, help='Number of using threads')
...
def conversion_for_train_file(file_name, encode_df, output_path, image_size):
    ...
    encode = list(encode_df.loc[encode_df['ImageId']
                  == fn_wo_ext, 'EncodedPixels'].values)
    if encode == []:
        return
    encode = get_mask(encode, image.shape[1], image.shape[0])
    image = resize(image, (image_size, image_size))

    cv2.imwrite(os.path.join(output_path, 'train',
                f'{fn_wo_ext}.png'), (image * 255).astype('uint8'))
...
def conversion_for_test_file(file_name, output_path, image_size):
    ...
    fn_wo_ext = file_name.split(os.path.sep)[-1][:-4]
    image = resize(image, (image_size, image_size))
    cv2.imwrite(os.path.join(output_path, 'test',
                f'{fn_wo_ext}.png'), (image * 255).astype('uint8'))
...
if __name__ == '__main__':
    ...
    conversion_for_train(
        train_fns, rle, args['output_path'], args['image_size'], n_threads=args['number_threads'])
