...
class BaseMattingDataset(VisionDataset):
    ...
    def __init__(self, metainfo, dataset_dir, transform, target_transform,
                 ...
                 data_prefix: dict = dict(
                     img_path='img', ann_path='ann', trimap_path='trimap_pth'),
                 return_dict=False):
        '''
        ...
        self.img_path = os.path.join(
            dataset_dir, data_prefix['img_path'], image_set)
        self.ann_path = os.path.join(
            dataset_dir, data_prefix['ann_path'], image_set)
        ...
        if self.has_trimap:
            self.trimap_path = os.path.join(
                dataset_dir, data_prefix['trimap_pth'], image_set)
            print('trimap_folder_name: {trimap_folder_name}'.format(
                trimap_folder_name=self.trimap_path))
    ...
    def __getitem__(self, index):
        img = Image.open(os.path.join(
            self.img_path, self.img_names[index] + self.img_suffix))
        ann = Image.open(os.path.join(
            self.ann_path, self.img_names[index] + self.ann_suffix))
        ...
        if self.has_trimap:
            ...
            trimap = Image.open(os.path.join(
                self.trimap_path, self.img_names[index] + self.trimap_suffix))
